...
const defaultHomeImage = (report) => {
  const itemText = `${report.itemName || ''} ${report.category || ''}`.toLowerCase();
  if (/\bcollege\s*(?:id|identity)\b/.test(itemText)) return 'images/collegeid.jpeg';
  if (/\b(?:identity\s*card|id\s*card|student\s*id)\b/.test(itemText)) return 'images/document.jpeg';
  if (/\bkeys?\b|\bkeychain\b/.test(itemText)) return 'images/keys.jpeg';
  if (/\b(?:earbuds|earphones)\b/.test(itemText)) return 'images/earbuds.jpeg';
  return '';
};
const persistedItemCard = (report, useDefaultImage = false) => {
  const type = report.type === 'found' ? 'found' : 'lost';
  const imageUrl = report.imageUrl || (useDefaultImage ? defaultHomeImage(report) : '');
  const visual = imageUrl ? `<img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(report.itemName)}">` : (type === 'found' ? '✓' : '○');
...
function found() {
  ...
  form.onsubmit = async (event) => {
    ...
    } catch (error) { console.error('Unable to submit found report:', error); toast('Unable to submit your found item. Please try again.'); submitButton.disabled = false; submitButton.textContent = 'Submit found item'; }
  };
}
